...
#Based on code written by Bernhard Misof, ZFMK, Bonn
...
sub colour_code {
	...
	my %colours = ( 
		...
		$code{'2'}= ['< 0.02', $colours{'c2'}] ;
		$code{'3'}= ['< 0.04', $colours{'c3'}] ;
		$code{'4'}= ['< 0.07', $colours{'c4'}] ;
		$code{'5'}= ['< 0.11', $colours{'c5'}] ;
...
sub colour_rectangle {
	...
	for ($$ref_rect){
        ...
        $colour=$colours{'c2'}  and last if $$ref_rect < 0.02;
        $colour=$colours{'c3'}  and last if $$ref_rect < 0.04;
        $colour=$colours{'c4'}  and last if $$ref_rect < 0.07;
        $colour=$colours{'c5'}  and last if $$ref_rect < 0.11;
